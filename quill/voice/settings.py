"""Voice / speech settings for Quill.
...
def load_voice_settings(
    ...
    stt_key = pick(
        ["OPENAI_API_KEY", "QUILL_STT_API_KEY"],
        "stt_api_key",
        None,
        "CloudApiKey",
    )
    ...
    try:
        mic = int(pick(["QUILL_MIC_DEVICE"], "microphone_device", -1, "MicrophoneDeviceNumber"))
    except Exception:
        mic = -1

    style_key = str(pick(["QUILL_TTS_STYLE"], "voice_style", DEFAULT_TTS_STYLE, None)).strip().lower()
    ...
    voice_override = pick(["QUILL_TTS_VOICE"], "tts_voice", None, None)
    rate_override = pick(["QUILL_TTS_RATE"], "tts_rate", None, None)
    pitch_override = pick(["QUILL_TTS_PITCH"], "tts_pitch", None, None)

    settings = VoiceSettings(
        tts_enabled=_as_bool(pick(["QUILL_TTS"], "tts_enabled", True, None), True),
        stt_enabled=_as_bool(pick(["QUILL_STT"], "stt_enabled", True, None), True),
