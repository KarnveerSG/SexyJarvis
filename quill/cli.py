...
from .init_cmd import generate_quill_md
from .integrations import integrations_summary, list_integrations
from .extras import (
    archive_to_history,
    autosave_path,
    detect_test_command,
...
Slash commands:
  ...
  /integrations    Show connected third-party integrations
  ...
  /init            Generate a starter QUILL.md from the workspace
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="quill",
        description="Quill — an OpenHands-style interactive terminal AI coding agent.",
    )
    ...
    p.add_argument("--no-memory", action="store_true", help="Do not load CLAUDE.md/QUILL.md instruction files.")
...
def _probe_cursor(cfg) -> str | None:
    ...
    try:
        ...
        if "missing_api_key" in msg.lower() or "cursor_api_key" in msg.lower():
            return (
                "CURSOR_API_KEY missing or invalid. Set in %USERPROFILE%\\.quill\\.env "
...
def _validate_provider_keys(cfg) -> str | None:
    ...
    if provider == "cursor":
        if not cfg.has_cursor_key:
            return (
                "Cursor requires CURSOR_API_KEY in %USERPROFILE%\\.quill\\.env "
    ...
    if provider == "anthropic" and not cfg.has_key:
        return "Anthropic provider requires ANTHROPIC_API_KEY in env or ~/.quill/.env."
    if provider == "local":
    ...
    if cfg.provider == "auto" and not cfg.has_cursor_key and not cfg.has_key and not cfg.has_local:
        return (
            ...
            "~/.quill/.env, or start a local LLM (LM Studio/Ollama)."
        )
    return None


def _ensure_global_config_hint(ui: UI) -> None:
    global_dir = Path.home() / ".quill"
...
def run_repl(cfg, no_memory: bool, ui: UI, initial_task: str | None, voice=None, resume: bool = False) -> int:
    ...
    if cfg.provider == "auto" and not cfg.has_cursor_key and cfg.active_provider == "anthropic":
        ui.info(
            ...
            "Add key to %USERPROFILE%\\.quill\\.env (Cursor Dashboard → API Keys)."
        )
        ...
        while True:
            ...
            if user_input.startswith("/"):
                ...
                if cmd == "tools":
                    ...
                    continue
                if cmd == "integrations":
                    ui.info(f"Integrations: {integrations_summary()} (keys in ~/.quill/.env)")
                    for row in list_integrations():
                        mark = "✓" if row["status"] == "connected" else "○"
                        ui.print(f"  {mark} {row['name']}: {row['status']}")
                    continue
                if cmd == "retries":
                    if rest.isdigit():
                ...
                if cmd == "confirm":
                    ...
                    continue
                if cmd == "save":
                    target = Path(rest or "QUILL_session.json")
                ...
                if cmd == "init":
                    ...
                    path, created, msg = generate_quill_md(cfg.workspace, overwrite=overwrite)
                ...
                if cmd == "commands":
                    ...
                    if not customs:
                        ui.info("No user commands. Add .md files under .quill/commands/.")
                    else:
                ...
                if cmd == "retry":
                    ...
                    continue
                if cmd == "export":
                    target = Path(rest or f"QUILL_session_{int(__import__('time').time())}.md")
                    if not target.is_absolute():
                        target = cfg.workspace / target
                ...
                if cmd == "stats":
                    ...
                    if rest in ("on", "true", "1", "yes"):
                        cfg.telemetry = True
                        ui.info("Telemetry enabled (local-only, .quill/telemetry.jsonl).")
                    elif rest in ("off", "false", "0", "no"):
                        cfg.telemetry = False
