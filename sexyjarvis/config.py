"""Configuration loading for SexyJarvis.
...
def load_config(workspace: Path | None = None, overrides: dict | None = None) -> Config:
    ...
    # Layer 3: .env in workspace, then global ~/.sexyjarvis/.env
    dotenv = _parse_dotenv(ws / ".env")
    global_dotenv = _parse_dotenv(global_dir / ".env")
    # Layer 4: config.toml in workspace + .sexyjarvis/config.toml profile + global config
    base_toml = _load_toml(ws / "config.toml")
    profile_toml = _load_toml(ws / ".sexyjarvis" / "config.toml")
    ...
    toml_sj = toml_cfg.get("sexyjarvis", toml_cfg) if isinstance(toml_cfg, dict) else {}
    ...
    def pick(env_keys: list[str], toml_key: str, default):
        ...
        if toml_key in toml_sj:
            return toml_sj[toml_key]
        return default

    api_key = pick(["ANTHROPIC_API_KEY", "SEXYJARVIS_API_KEY"], "api_key", None)
    cursor_api_key = pick(["CURSOR_API_KEY", "SEXYJARVIS_CURSOR_API_KEY"], "cursor_api_key", None)
    model = str(pick(["SEXYJARVIS_MODEL", "ANTHROPIC_MODEL"], "model", DEFAULT_MODEL))
    cursor_model = str(pick(["SEXYJARVIS_CURSOR_MODEL", "CURSOR_MODEL"], "cursor_model", DEFAULT_CURSOR_MODEL))
    provider = str(pick(["SEXYJARVIS_PROVIDER"], "provider", DEFAULT_PROVIDER)).lower()
    fallback_model = str(pick(["SEXYJARVIS_FALLBACK_MODEL"], "fallback_model", DEFAULT_MODEL))
    local_base_url = pick(
        ["SEXYJARVIS_LOCAL_URL", "LM_STUDIO_URL", "OLLAMA_HOST"],
        "local_base_url",
        None,
    )
    local_model = str(pick(["SEXYJARVIS_LOCAL_MODEL", "LM_MODEL", "OLLAMA_MODEL"], "local_model", DEFAULT_LOCAL_MODEL))
    base_url = pick(["ANTHROPIC_BASE_URL", "SEXYJARVIS_BASE_URL"], "base_url", None)
    ...
    cfg = Config(
        ...
        fallback_enabled=as_bool(pick(["SEXYJARVIS_FALLBACK"], "fallback_enabled", True), True),
        local_base_url=str(local_base_url).strip() if local_base_url else None,
        local_model=local_model,
        rtk_enabled=as_bool(pick(["SEXYJARVIS_RTK"], "rtk_enabled", True), True),
        codegraph_enabled=as_bool(pick(["SEXYJARVIS_CODEGRAPH"], "codegraph_enabled", True), True),
        max_tokens=as_int(pick(["SEXYJARVIS_MAX_TOKENS"], "max_tokens", DEFAULT_MAX_TOKENS), DEFAULT_MAX_TOKENS),
        max_retries=as_int(pick(["SEXYJARVIS_MAX_RETRIES"], "max_retries", DEFAULT_MAX_RETRIES), DEFAULT_MAX_RETRIES),
        retry_base_delay=as_float(pick(["SEXYJARVIS_RETRY_DELAY"], "retry_base_delay", DEFAULT_RETRY_BASE_DELAY), DEFAULT_RETRY_BASE_DELAY),
        bash_timeout=as_int(pick(["SEXYJARVIS_BASH_TIMEOUT"], "bash_timeout", DEFAULT_BASH_TIMEOUT), DEFAULT_BASH_TIMEOUT),
